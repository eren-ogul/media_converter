...
cls
echo ==================================================
echo     VIDEO COZUNURLUK DUSURUCU (480p - %%100 GPU)
echo ==================================================
echo.
...
for %%i in ("input\*.mp4" "input\*.mkv" "input\*.webm" "input\*.avi" "input\*.mov" "input\*.flv") do (
    ...
    ffmpeg -hwaccel cuda -hwaccel_output_format cuda -i "%%i" -vf "scale_cuda=-2:480:format=yuv420p" -c:v h264_nvenc -cq 28 -c:a copy "output\%%~ni_!sayac!(480p).mp4"
